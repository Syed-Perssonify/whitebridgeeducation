...
const REGION_COUNTRIES: Record<string, string[]> = {
  southAsia: [
    'India', 'Nepal', 'Bangladesh', 'Sri Lanka', 'Pakistan', 'Bhutan', 'Maldives'
  ],
  middleEast: [
    'Saudi Arabia', 'United Arab Emirates', 'Oman', 'Qatar', 'Kuwait',
    'Bahrain', 'Yemen', 'Iraq', 'Jordan', 'Lebanon', 'Syria', 'Israel'
  ],
  centralAsia: [
    'Kazakhstan', 'Uzbekistan', 'Turkmenistan', 'Tajikistan', 'Kyrgyzstan'
  ],
  westAsia: [
    'Turkey', 'Iran', 'Azerbaijan', 'Georgia', 'Armenia', 'Afghanistan'
  ],
};
...
const getCountryRegion = (countryName: string, continent?: string): string | null => {
...
const getRegionColorDynamic = (feature: any, activeRegions: Set<string>): string => {
  ...
  if (region && activeRegions.has(region)) {
    return REGION_COLORS[region as keyof typeof REGION_COLORS] || REGION_COLORS.default;
  }
...
export function GlobePremium({ globeConfig, data }: WorldProps) {
  ...
  const rotationPhase = useRef<'center' | 'toDestination' | 'atDestination'>('center');
  ...
  useEffect(() => {
    ...
    globeRef.current
      ...
      .polygonCapColor((feature: any) => getRegionColorDynamic(feature, activeRegions))
  ...
  const calculateDestinationRotationY = (destLng: number): number => {
    ...
    rotationOffset = Math.max(-MAX_ROTATION_OFFSET, Math.min(MAX_ROTATION_OFFSET, rotationOffset));
  ...
  const calculateDestinationRotationX = (destLat: number): number => {
    ...
    tiltOffset = Math.max(-MAX_TILT_OFFSET, Math.min(MAX_TILT_OFFSET, tiltOffset));
  ...
  const areSimilarDirections = (
    ...
    const prevAngle = Math.atan2(prevLat - INDIA_LAT, prevLng - INDIA_LNG) * (180 / Math.PI);
    const newAngle = Math.atan2(newLat - INDIA_LAT, newLng - INDIA_LNG) * (180 / Math.PI);
  ...
  useEffect(() => {
    ...
    const destRegion = currentArc.label ? DESTINATION_TO_REGION[currentArc.label] : null;
    const destColor = getRegionSolidColor(destRegion);
